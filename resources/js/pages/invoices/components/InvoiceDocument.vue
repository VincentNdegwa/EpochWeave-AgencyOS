    ...
    <div class="mx-auto w-full max-w-4xl bg-white text-slate-900">
        <div class="flex items-start justify-between px-5 pt-10 pb-8">
            <div class="space-y-1">
                ...
                <p
                    v-if="workspace?.address"
                    class="text-xs leading-relaxed text-slate-500"
                >
                    {{ workspace.address }}
                </p>
                <p v-if="workspace?.email" class="text-xs text-slate-500">
                    {{ workspace.email }}
                </p>
                <p v-if="workspace?.phone" class="text-xs text-slate-500">
                    {{ workspace.phone }}
                </p>
                <p v-if="workspace?.tax_number" class="text-xs text-slate-500">
                    Tax No: {{ workspace.tax_number }}
                </p>
            </div>
            <div class="space-y-1 text-right">
                ...
                <p class="font-mono text-base font-semibold text-slate-700">
                    {{ invoiceNumber }}
                </p>
                <Badge
                    v-if="dueStatus"
        ...
        <div
            class="mx-5 mb-8 grid grid-cols-3 divide-x divide-slate-200 overflow-hidden rounded-xl border border-slate-200 bg-slate-50"
        >
            <div class="px-5 py-4">
                <p
            ...
            <div class="px-5 py-4">
                ...
                <p class="mt-1 text-sm font-semibold text-slate-800 capitalize">
                    {{ invoice.invoice_status?.title ?? 'Draft' }}
                </p>
            </div>
        </div>
        ...
        <div class="mx-5 mb-8 grid grid-cols-2 gap-8">
            <div class="space-y-1">
                ...
                <template v-if="contact">
                    ...
                    <p v-if="contact.job_title" class="text-xs text-slate-500">
                        {{ contact.job_title }}
                    </p>
                    <p class="text-xs text-slate-500">{{ contact.email }}</p>
                    <p v-if="contact.phone" class="text-xs text-slate-500">
                        {{ contact.phone }}
                    </p>
                </template>
            </div>
        ...
        <div class="mx-5 mb-6">
            <div
                class="grid grid-cols-[1fr_60px_100px_100px] gap-3 rounded-t-md bg-slate-900 px-4 py-3 text-[11px] font-semibold tracking-widest text-white uppercase"
            >
            ...
            <div
                class="divide-y divide-slate-100 overflow-hidden rounded-b-md border border-t-0 border-slate-200"
            >
                <div
                    v-for="item in invoice.items ?? []"
                    :key="item.id"
                    class="grid grid-cols-[1fr_60px_100px_100px] items-start gap-3 px-4 py-4"
                >
                    <div>
                    ...
                    <p class="text-center text-sm text-slate-700">
                        {{ item.quantity
                        }}<span v-if="item.unit_label" class="text-slate-400">
                            {{ item.unit_label }}</span
                        >
                    </p>
        ...
        <div class="mx-5 mb-8 flex justify-end">
            <div class="w-72 space-y-2 text-sm">
                ...
                <template
                    v-if="invoice.invoice_status?.automation_trigger === 'paid'"
                >
                    <div
                        class="flex justify-between font-medium text-green-600"
                    >
                        <span>Paid</span>
                        <span>{{ f(invoice.grand_total) }}</span>
                    </div>
        ...
        <div
            v-if="invoice.notes || workspace?.payment_instructions"
            class="mx-5 mb-10 space-y-4"
        >
            <Separator class="bg-slate-200" />
            <div
                v-if="workspace?.payment_instructions"
                class="rounded-md border border-slate-200 bg-slate-50 px-5 py-4"
            >
                <p
                    class="mb-2 text-[10px] font-semibold tracking-widest text-slate-400 uppercase"
                >
                    Payment instructions
                </p>
                <p
                    class="text-sm leading-relaxed whitespace-pre-line text-slate-700"
                >
