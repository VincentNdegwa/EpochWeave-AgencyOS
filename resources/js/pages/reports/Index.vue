...
const revenueTotal = computed(() => props.revenueByMonth.reduce((a, b) => a + b, 0));
const hoursTotal = computed(() => props.hoursByMonth.reduce((a, b) => a + b, 0));
...
<template>
    ...
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
                <Button variant="outline" size="sm" @click="changeYear(-1)">{{ year - 1 }}</Button>
        ...
        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <div class="rounded-lg border bg-card p-4">
                ...
                <p class="text-2xl font-bold">{{ revenueTotal.toLocaleString(undefined, { style: 'currency', currency: 'USD' }) }}</p>
            </div>
            <div class="rounded-lg border bg-card p-4">
            ...
            <div class="rounded-lg border bg-card p-4">
                ...
                <p class="text-2xl font-bold">{{ proposalStatusCounts.reduce((a, b) => a + b.count, 0) }}</p>
            </div>
            <div class="rounded-lg border bg-card p-4">
                <p class="text-xs text-muted-foreground">Top Client</p>
                <p class="text-lg font-bold truncate">{{ topClients[0]?.name ?? '—' }}</p>
            </div>
        </div>
        ...
        <div class="grid gap-4 lg:grid-cols-2">
                ...
                <div class="flex items-end gap-1 h-48">
                    ...
                    >
                        <div
                            ...
                            :style="{ height: `${(value / (Math.max(...revenueByMonth) || 1)) * 100}%` }"
                        />
            ...
            <div class="rounded-lg border bg-card p-4">
                ...
                <div class="space-y-2">
                    ...
                    <p v-if="!proposalStatusCounts.length" class="text-sm text-muted-foreground">No proposals yet.</p>
                </div>
            </div>
            ...
            <div class="rounded-lg border bg-card p-4">
                ...
                <div class="space-y-2">
                    ...
                    >
                        ...
                        <span class="text-sm font-medium">{{ client.total.toLocaleString(undefined, { style: 'currency', currency: 'USD' }) }}</span>
                    </div>
                    <p v-if="!topClients.length" class="text-sm text-muted-foreground">No invoices yet.</p>
                </div>
            </div>
                ...
                <div class="flex items-end gap-1 h-48">
                    ...
                    >
                        <div
                            ...
                            :style="{ height: `${(value / (Math.max(...hoursByMonth) || 1)) * 100}%` }"
                        />
                        <span class="text-[10px] text-muted-foreground">{{ monthLabels[index] }}</span>
                    </div>
                </div>
