...
const colWidthStyle = (colIndex: number) => {
    ...
    return {
        flex: `0 0 ${widths[colIndex] ?? 50}%`,
        maxWidth: `${widths[colIndex] ?? 50}%`,
    };
};
    ...
    <section class="px-8 py-6">
        ...
        >
            ...
            >
                ...
                <div v-if="!isLocked" class="mb-2 flex items-center gap-1.5">
                ...
                <div class="space-y-2">
                    <div v-for="childBlock in column" :key="childBlock.id">
                        ...
                        >
                            ...
                            <button
                                v-if="!isLocked"
                                type="button"
                                class="absolute top-1 right-1 z-10 flex h-5 w-5 items-center justify-center rounded border border-border bg-background/90 text-muted-foreground opacity-0 shadow transition group-hover/child:opacity-100 hover:text-destructive"
                                @click="
                    ...
                    <div v-if="!isLocked">
                        ...
                        <div
                            ...
                            class="flex flex-col items-center justify-center gap-2 rounded-lg border-2 border-dashed border-border bg-muted/20 py-8 text-center"
                        >
                        ...
                        <div class="relative mt-1 flex justify-center">
                            <details class="group/add">
                                <summary
                                    class="flex cursor-pointer list-none items-center gap-1.5 rounded-md border border-dashed border-border px-3 py-1.5 text-xs text-muted-foreground transition hover:border-primary hover:text-foreground"
                                >
                                    <PlusIcon class="h-3 w-3" />
                                    Add block
                                </summary>
                                <div
                                    class="absolute left-1/2 z-30 mt-1 w-44 -translate-x-1/2 overflow-hidden rounded-lg border border-border bg-background shadow-lg"
                                >
                                    <button
                                        v-for="type in allowedChildTypes"
                                        :key="type"
                                        type="button"
                                        class="flex w-full items-center px-3 py-2 text-left text-xs text-muted-foreground transition hover:bg-muted hover:text-foreground"
                                        @click="
                                            addBlockToColumn(
                                                colIndex,
                                                type as BaseBlock['type'],
                                            )
                                        "
                                    >
                                        {{ type.replace(/_/g, ' ') }}
