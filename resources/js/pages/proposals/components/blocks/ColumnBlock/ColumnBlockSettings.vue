...
const block = computed(
    () => store.blocks.find((b) => b.id === props.blockId) ?? null,
);
const data = computed<ColumnBlockData | null>(
    () => (block.value?.data as ColumnBlockData) ?? null,
);
...
<template>
    <div v-if="data" class="flex flex-col gap-0 text-sm">
        ...
        <div class="border-b border-border py-3">
            ...
            <div class="grid grid-cols-3 gap-1.5">
                <button
                    ...
                    :key="opt.value"
                    type="button"
                    class="flex flex-col items-center gap-1.5 rounded-lg border px-2 py-3 text-xs font-medium transition"
                    :class="
        ...
        <div v-if="data.columns === 2" class="border-b border-border py-3">
            <p
            ...
            <div class="mb-2 grid grid-cols-2 gap-1">
                <button
                    ...
                    :class="
                        JSON.stringify(data.column_widths) ===
            ...
            >
                <div
                    class="flex items-center justify-center bg-primary/10 py-1.5 text-[11px] font-medium text-primary"
                    :style="{ width: `${data.column_widths[0]}%` }"
                >
                    {{ data.column_widths[0] }}%
                </div>
                <div
                    class="flex flex-1 items-center justify-center bg-muted/50 py-1.5 text-[11px] font-medium text-muted-foreground"
                >
        ...
        <div class="border-b border-border py-3">
            ...
            <div class="grid grid-cols-3 gap-1">
                <button
                    ...
                    :key="opt.value"
                    type="button"
                    class="flex flex-col items-center gap-1 rounded-md border py-2 text-xs font-medium transition"
                    :class="
                        (data.gap ?? 'md') === opt.value
                            ? 'border-primary bg-primary/5 text-primary'
                            : 'border-border bg-background text-muted-foreground hover:border-muted-foreground'
                    "
        ...
        <div class="py-3">
            ...
            <div class="grid grid-cols-3 gap-1">
                <button
                    ...
                    class="rounded-md border py-1.5 text-center text-xs font-medium transition"
                    :class="
                        (data.vertical_align ?? 'start') === opt.value
                            ? 'border-primary bg-primary/5 text-primary'
                            : 'border-border bg-background text-muted-foreground hover:border-muted-foreground'
                    "
            ...
            <p class="mt-2 text-[11px] text-muted-foreground">
                Controls how columns align vertically when they have different
                heights.
            </p>
        </div>
    </div>
