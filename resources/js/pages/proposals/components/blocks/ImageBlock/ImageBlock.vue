...
const hasImages = computed(() =>
    props.data.images.some((img) => img.url.length > 0),
);
</script>
...
<template>
    <section class="px-6">
        ...
        <template v-if="hasImages || isLocked">
                ...
                <template v-for="img in data.images" :key="img.id">
                    <div v-if="img.url" class="group/img relative">
                        ...
                        <button
                            v-if="!isLocked"
                            type="button"
                            class="absolute top-2 right-2 flex h-6 w-6 items-center justify-center rounded-full bg-background/90 text-muted-foreground opacity-0 shadow transition group-hover/img:opacity-100 hover:text-destructive"
            ...
            <div v-else :class="gridClass">
                ...
                >
                    ...
                    <label
                        v-else-if="!isLocked"
                        class="flex cursor-pointer flex-col items-center justify-center gap-2 rounded-lg border-2 border-dashed border-border bg-muted/30 p-6 text-center transition hover:border-primary hover:bg-muted/50"
                        :class="[
                            aspectClass,
                            dragOverId === img.id
                ...
                <button
                    v-if="!isLocked"
                    type="button"
                    class="flex flex-col items-center justify-center gap-2 rounded-lg border-2 border-dashed border-border bg-muted/20 p-6 text-center transition hover:border-primary hover:bg-muted/40"
                    :class="aspectClass"
                    @click="addImage"
                >
        ...
        <label
            v-else-if="!isLocked"
            class="group/drop flex cursor-pointer flex-col items-center justify-center gap-3 rounded-xl border-2 border-dashed border-border bg-muted/30 px-8 py-14 text-center transition hover:border-primary hover:bg-muted/50"
            ...
            @drop="
                (e) => {
                    ...
                    if (file?.type.startsWith('image/')) {
                        addImage();
                        handleDrop(
                            data.images[data.images.length - 1]?.id ?? '',
                            e,
                        );
                    }
                }
        ...
        >
            <div
                class="flex h-12 w-12 items-center justify-center rounded-full border border-border bg-background text-muted-foreground shadow-sm transition group-hover/drop:border-primary group-hover/drop:text-primary"
            >
