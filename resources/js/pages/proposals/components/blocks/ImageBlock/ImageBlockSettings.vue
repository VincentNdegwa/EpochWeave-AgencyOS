...
const expandedImageId = ref<string | null>(data.value?.images?.[0]?.id ?? null);
...
<template>
    ...
    ══════════════════════════════════════════════ -->
        <div class="border-b border-border py-3">
            ...
            >
                ...
                Images
                <span
                    class="ml-auto text-[11px] font-normal text-muted-foreground"
                >
                    {{ data.images.length }} /
                    {{ data.columns === 1 ? 1 : data.columns === 2 ? 4 : 6 }}
                </span>
            </p>
            ...
            <div class="space-y-2">
                <Collapsible
                    ...
                    @update:open="
                        (open) => {
                            expandedImageId = open ? img.id : null;
                        }
                    "
                >
                    <div
                    ...
                    >
                        <!-- Accordion header -->
                        <CollapsibleTrigger
                            class="flex w-full items-center gap-2 bg-muted/30 px-3 py-2.5 transition-colors hover:bg-muted/60"
                        >
                            <!-- Thumbnail preview -->
                            <div
                                class="h-7 w-10 flex-shrink-0 overflow-hidden rounded border border-border bg-muted"
                            >
                                <img
                                    v-if="img.url"
                                    :src="img.url"
                            ...
                            <span
                                class="flex-1 text-left text-xs font-medium text-foreground"
                            >
                            ...
                            <div class="flex items-center gap-1">
                                ...
                                <button
                                    ...
                                    type="button"
                                    class="flex h-5 w-5 items-center justify-center rounded text-muted-foreground transition-colors hover:text-destructive"
                        ...
                        <CollapsibleContent>
                            <div
                                class="space-y-3 border-t border-border bg-background p-3"
                            >
                                <!-- URL / Upload toggle -->
                                <div
                                    class="flex gap-1 rounded-md border border-border bg-muted/40 p-0.5"
                                >
                                    <button
                                        type="button"
                                        class="flex flex-1 items-center justify-center gap-1.5 rounded py-1.5 text-xs font-medium transition"
                                        :class="
                                            getMode(img.id) === 'upload'
                                ...
                                <template v-if="getMode(img.id) === 'upload'">
                                    <label
                                        class="relative flex cursor-pointer flex-col items-center justify-center gap-1.5 rounded-lg border-2 border-dashed border-border bg-muted/30 py-4 text-center transition hover:border-primary hover:bg-muted/50"
                                    >
                                        <span
                                    ...
                                    >
                                        <img
                                            :src="img.url"
                                            :alt="img.alt_text ?? ''"
                                            class="h-8 w-12 flex-shrink-0 rounded border border-border object-cover"
                                        />
                                        <span
                                            class="flex-1 truncate text-[11px] text-muted-foreground"
                                        >
                                            Image uploaded
                                        </span>
                                        <button
            ...
            <Button
                ...
                variant="outline"
                size="sm"
                class="mt-3 w-full gap-1.5 border-dashed text-xs"
                @click="addImage"
            >
    ...
    ══════════════════════════════════════════════ -->
        <div class="border-b border-border py-3">
            ...
            <div v-if="(data.columns ?? 1) === 1" class="mb-4 grid gap-2">
                ...
                <div class="grid grid-cols-4 gap-1">
                    <button
                        ...
                        :class="
                            (data.alignment ?? 'center') === opt.value
                                ? 'border-primary bg-primary/5 text-primary'
            ...
            <div class="mb-4 grid gap-2">
                ...
                <div class="grid grid-cols-4 gap-1">
                    <button
                        ...
                        class="rounded-md border py-1.5 text-center text-xs font-medium transition"
                        :class="
                            (data.aspect_ratio ?? 'auto') === opt.value
                                ? 'border-primary bg-primary/5 text-primary'
                                : 'border-border bg-background text-muted-foreground hover:border-muted-foreground'
                        "
            ...
            >
                ...
                >
                    ...
                    <Switch
                        ...
                        @update:model-value="
                            (v: boolean) => updateData({ rounded: v })
                        "
                    />
                </label>
                <label
                ...
                >
                    <div>
                        ...
                        <p class="text-[11px] text-muted-foreground">
                            Display caption text below each image
                        </p>
                    </div>
                    <Switch
                        :model-value="data.show_captions ?? true"
                        @update:model-value="
                            (v: boolean) => updateData({ show_captions: v })
                        "
                    />
                </label>
            </div>
