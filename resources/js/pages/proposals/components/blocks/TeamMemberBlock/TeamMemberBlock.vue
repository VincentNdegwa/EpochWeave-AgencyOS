...
import type { BaseBlock, TeamMemberBlockData, TeamMember } from '@/types/proposal-builder';
...
const updateMember = (id: string, changes: Partial<TeamMember>) => {
  updateData({
    members: props.data.members.map((m) => m.id === id ? { ...m, ...changes } : m),
  });
};
...
const addMember = () => {
  updateData({
    members: [
      ...props.data.members,
      { id: nanoid(8), name: 'Team Member', role: 'Role', bio: null, avatar_url: null, linkedin_url: null },
    ],
  });
};
...
return;
  ...
  reader.onload = () => updateMember(id, { avatar_url: typeof reader.result === 'string' ? reader.result : null });
...
<template>
  <section class="px-8 py-6">
    ...
    >
      ...
      >
        <!-- Avatar -->
        <label :class="['relative mx-auto mb-3 block h-20 w-20 overflow-hidden rounded-full border-2 border-border', !isLocked ? 'cursor-pointer' : '']">
          <img
          ...
          <input v-if="!isLocked" type="file" accept="image/*" class="sr-only" @change="(e) => handleAvatarUpload(member.id, e)" />
        </label>
        ...
        <div class="text-center">
          <input
            ...
            @input="updateMember(member.id, { name: ($event.target as HTMLInputElement).value })"
          />
          <p v-else class="text-sm font-semibold text-foreground">{{ member.name }}</p>
          ...
          <input
            ...
            @input="updateMember(member.id, { role: ($event.target as HTMLInputElement).value })"
          />
          <p v-else class="text-xs text-muted-foreground">{{ member.role }}</p>
          ...
          <textarea
            ...
            @input="updateMember(member.id, { bio: ($event.target as HTMLTextAreaElement).value || null })"
          />
          <p v-else-if="member.bio" class="mt-1 text-xs text-muted-foreground">{{ member.bio }}</p>
        ...
        <button
          v-if="!isLocked"
          type="button"
          class="absolute right-0 top-0 flex h-5 w-5 items-center justify-center rounded-full
                 bg-background shadow border border-border text-muted-foreground
                 opacity-0 transition hover:text-destructive group-hover/member:opacity-100"
    ...
    <div v-else class="space-y-4">
      ...
      >
        <!-- Avatar -->
        <label :class="['relative h-14 w-14 flex-shrink-0 overflow-hidden rounded-full border border-border', !isLocked ? 'cursor-pointer' : '']">
        ...
        <button
          v-if="!isLocked"
          type="button"
          class="absolute right-3 top-3 flex h-5 w-5 items-center justify-center
                 rounded text-muted-foreground opacity-0 transition hover:text-destructive
      ...
      <button
        v-if="!isLocked"
        type="button"
        class="flex w-full items-center justify-center gap-2 rounded-lg border-2 border-dashed
               border-border py-3 text-xs text-muted-foreground hover:border-primary
