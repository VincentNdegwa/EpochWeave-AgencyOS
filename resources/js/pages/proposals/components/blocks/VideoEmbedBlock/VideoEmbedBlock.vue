...
<template>
    <section class="px-8 py-6">
        ...
        <div v-if="hasVideo" class="space-y-3">
            <div
                class="aspect-video overflow-hidden rounded-xl border border-border"
            >
                <iframe
                    :src="embedUrl!"
            ...
            <div v-if="data.title || data.caption" class="space-y-0.5">
                ...
                <p v-if="data.caption" class="text-xs text-muted-foreground">
                    {{ data.caption }}
                </p>
            </div>
        </div>
        ...
        <div
            v-else-if="!isLocked"
            class="flex flex-col items-center justify-center gap-3 rounded-xl border-2 border-dashed border-border bg-muted/30 px-8 py-12 text-center"
        >
            <div
                class="flex h-12 w-12 items-center justify-center rounded-full border border-border bg-background text-muted-foreground"
            >
            ...
            <div>
                ...
                <p class="mt-0.5 text-xs text-muted-foreground">
                    YouTube, Vimeo, or a direct link
                </p>
            </div>
            <div
                class="flex w-full max-w-sm items-center gap-2 rounded-lg border border-border bg-background px-3"
            >
                <LinkIcon
        ...
        <div
            v-else
            class="flex flex-col items-center justify-center gap-2 rounded-xl border border-border py-10 text-center"
        >
