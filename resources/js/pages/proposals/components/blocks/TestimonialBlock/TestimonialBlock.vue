...
const updateItem = (id: string, changes: Partial<Testimonial>) => {
  updateData({
    items: props.data.items.map((t) => t.id === id ? { ...t, ...changes } : t),
  });
};
    ...
    <template v-else-if="data.layout === 'single'">
      ...
      >
        ...
        <textarea
          ...
          :value="item.quote"
          rows="3"
          class="mb-6 w-full resize-none border-none bg-transparent text-lg italic
                 leading-relaxed text-foreground outline-none"
          @input="updateItem(item.id, { quote: ($event.target as HTMLTextAreaElement).value })"
        />
        <p v-else class="mb-6 text-lg italic leading-relaxed text-foreground">
          "{{ item.quote }}"
        </p>

        <div class="flex items-center gap-3">
          <label :class="['h-10 w-10 flex-shrink-0 overflow-hidden rounded-full border border-border', !isLocked ? 'cursor-pointer' : '']">
            <img v-if="item.avatar_url" :src="item.avatar_url" :alt="item.author_name" class="h-full w-full object-cover" />
            <div v-else class="flex h-full w-full items-center justify-center bg-muted">
              <UserIcon class="h-4 w-4 text-muted-foreground/40" />
            </div>
            <input v-if="!isLocked" type="file" accept="image/*" class="sr-only" @change="(e) => handleAvatarUpload(item.id, e)" />
          </label>
          <div>
            <input
              v-if="!isLocked"
              ...
              @input="updateItem(item.id, { author_name: ($event.target as HTMLInputElement).value })"
            />
            <p v-else class="text-sm font-semibold text-foreground">{{ item.author_name }}</p>

            <div v-if="!isLocked" class="flex gap-1">
              <input type="text" :value="item.author_role ?? ''" placeholder="Role"
                class="w-20 border-none bg-transparent text-xs text-muted-foreground outline-none"
                @input="updateItem(item.id, { author_role: ($event.target as HTMLInputElement).value || null })" />
              <span class="text-xs text-muted-foreground">·</span>
              <input type="text" :value="item.company_name ?? ''" placeholder="Company"
                class="w-24 border-none bg-transparent text-xs text-muted-foreground outline-none"
                @input="updateItem(item.id, { company_name: ($event.target as HTMLInputElement).value || null })" />
            </div>
          ...
          <button
            v-if="!isLocked"
            type="button"
            class="ml-auto text-muted-foreground opacity-0 transition hover:text-destructive group-hover/item:opacity-100"
      ...
      <button
        v-if="!isLocked"
        type="button"
        class="mt-6 flex w-full items-center justify-center gap-2 rounded-lg border-2
               border-dashed border-border py-3 text-xs text-muted-foreground
      ...
      <div class="grid grid-cols-2 gap-4">
        ...
        >
          ...
          <textarea
            ...
            :value="item.quote"
            rows="3"
            class="mb-4 w-full resize-none border-none bg-transparent text-sm italic
                   leading-relaxed text-foreground outline-none"
            @input="updateItem(item.id, { quote: ($event.target as HTMLTextAreaElement).value })"
          />
          <p v-else class="mb-4 text-sm italic leading-relaxed text-foreground">
            "{{ item.quote }}"
          </p>

          <div class="flex items-center gap-2.5">
            <label :class="['h-8 w-8 flex-shrink-0 overflow-hidden rounded-full border border-border', !isLocked ? 'cursor-pointer' : '']">
              <img v-if="item.avatar_url" :src="item.avatar_url" :alt="item.author_name" class="h-full w-full object-cover" />
              <div v-else class="flex h-full w-full items-center justify-center bg-muted">
                <UserIcon class="h-3.5 w-3.5 text-muted-foreground/40" />
              </div>
              <input v-if="!isLocked" type="file" accept="image/*" class="sr-only" @change="(e) => handleAvatarUpload(item.id, e)" />
            </label>
            <div class="min-w-0">
              <input
                ...
                @input="updateItem(item.id, { author_name: ($event.target as HTMLInputElement).value })"
              />
              <p v-else class="truncate text-xs font-semibold text-foreground">{{ item.author_name }}</p>
          ...
          <button
            v-if="!isLocked"
            type="button"
            class="absolute right-2 top-2 flex h-5 w-5 items-center justify-center rounded
        ...
        <button
          ...
          class="flex flex-col items-center justify-center gap-2 rounded-lg border-2
                 border-dashed border-border py-8 text-xs text-muted-foreground
                 hover:border-primary hover:text-foreground transition"
          @click="addItem"
        >
