    ...
    <div class="py-3 border-b border-border">
      ...
      <div class="grid gap-3">
        ...
        <div class="rounded-md border border-border bg-muted/30 px-4 py-3">
          <hr
            :style="{
              borderTopStyle: data.style === 'none' ? 'none' : data.style,
              borderTopWidth: `${data.thickness_px}px`,
              borderTopColor: data.color,
            }"
            class="border-0"
          />
          <p v-if="data.style === 'none'" class="text-center text-[11px] text-muted-foreground">No line</p>
        </div>

        <div v-if="data.style !== 'none'" class="grid gap-1.5">
          <Label class="text-xs text-muted-foreground">Thickness</Label>
          <div class="grid grid-cols-3 gap-1">
            <button
              ...
              type="button"
              class="rounded-md border py-1.5 text-center text-xs font-medium transition"
              :class="data.thickness_px === opt.value
                ? 'border-primary bg-primary/5 text-primary'
                : 'border-border bg-background text-muted-foreground hover:border-muted-foreground'"
