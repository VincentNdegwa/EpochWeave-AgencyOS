    ...
    <div v-if="data" class="flex flex-col gap-0 text-sm">
            ...
            <div class="space-y-3">
                ...
                <div class="grid gap-1.5">
                    ...
                    <textarea
                        ...
                        class="w-full resize-none rounded-md border border-input bg-background px-3 py-2 text-sm leading-relaxed placeholder:text-muted-foreground focus:ring-1 focus:ring-ring focus:outline-none"
                        @input="
                            (e) =>
                                updateData({
                                    subheading:
        ...
        <div class="border-b border-border py-3">
            ...
            <Tabs
                :model-value="bgTab"
                class="w-full"
                @update:modelValue="onBgTabChange"
            >
                <TabsList class="grid h-8 w-full grid-cols-2 rounded-md">
                    <TabsTrigger value="color" class="rounded-sm text-xs"
                        >Solid color</TabsTrigger
                    >
                    <TabsTrigger value="image" class="rounded-sm text-xs"
                        >Image</TabsTrigger
                    >
                </TabsList>
            </Tabs>
            ...
            <div v-if="bgTab === 'color'" class="mt-3">
                <ColorPresets
                    ...
                    placeholder="#0F172A"
                    @update:model-value="
                        (v) => updateData({ background_value: v })
                    "
                />
            </div>
            ...
            <div v-else class="mt-3 space-y-3">
                <div class="grid gap-1.5">
                    ...
                    <Input
                        type="url"
                        :model-value="
                            data.background_value?.startsWith('data:')
                                ? ''
                ...
                <div
                    v-if="
                        data.background_value &&
                        data.background_value.length > 0
                    "
                ...
                >
                    ...
                    <button
                        type="button"
                        class="absolute top-1.5 right-1.5 flex h-5 w-5 items-center justify-center rounded-full bg-background/80 text-muted-foreground hover:text-destructive"
