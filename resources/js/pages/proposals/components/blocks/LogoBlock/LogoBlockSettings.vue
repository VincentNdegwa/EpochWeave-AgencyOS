    ...
    <div class="py-3 border-b border-border">
      ...
      <div class="space-y-3">
        <div v-if="data.layout !== 'client_only'" class="grid gap-1.5">
          <Label class="text-xs text-muted-foreground">Workspace logo URL</Label>
          <Input
            type="url"
            :model-value="data.workspace_logo_url ?? ''"
            placeholder="https://example.com/logo.png"
            class="h-8 text-xs"
            @update:model-value="(v) => updateData({ workspace_logo_url: v ? String(v) : null })"
          />
        </div>

        <div v-if="data.layout !== 'workspace_only'" class="grid gap-1.5">
          <Label class="text-xs text-muted-foreground">Client logo URL</Label>
          <Input
            type="url"
            :model-value="data.client_logo_url ?? ''"
            placeholder="https://example.com/client-logo.png"
            class="h-8 text-xs"
            @update:model-value="(v) => updateData({ client_logo_url: v ? String(v) : null })"
          />
        </div>
      </div>
    </div>
