...
const displayProposalNumber = computed(
    () => proposal.value.proposal_number ?? proposal.value.token ?? 'DRAFT',
);
...
const selectedCurrency = computed(
    () =>
        currencies.find(
            (currency) => currency.value === proposal.value.currency,
        ) ?? currencies[0],
);

const today = new Date().toISOString().split('T')[0];
const isExpired = computed(() =>
    proposal.value.valid_until ? proposal.value.valid_until < today : false,
);
...
const daysUntil = computed(() => {
    ...
    return Math.ceil(
        (new Date(proposal.value.valid_until).getTime() - Date.now()) /
            86_400_000,
    );
});
...
const validityHint = computed(() => {
    ...
    return daysUntil.value !== null
        ? `Expires in ${daysUntil.value} days`
        : null;
});
...
const depositValueModel = computed({
    ...
    set: (value: number | string) => {
        ...
        proposal.value.deposit_value = Number.isFinite(numericValue)
            ? numericValue
            : 0;
    },
});
...
<template>
    <div class="flex flex-col gap-0 text-sm h-full">
        ...
        <div class="border-b border-border px-4 py-3">
            ...
            <div
                class="mb-2 flex items-center gap-2 rounded-md border border-border bg-background px-3 py-2"
            >
        ...
        <div class="border-b border-border px-4 py-3">
            ...
            >
                <AlertCircleIcon
                    v-if="isExpired || (daysUntil !== null && daysUntil <= 3)"
        ...
        <div class="px-4 py-3">
            ...
            >
                <div>
                    ...
                    <p class="text-[11px] text-muted-foreground">
                        Client pays before work begins
                    </p>
                </div>
                <Switch
                ...
                <div>
                    ...
                    >
                        <button
                            ...
                            :class="
                                proposal.deposit_type === 'percentage'
                    ...
                    <template v-if="proposal.deposit_type === 'percentage'">
                        <div class="mb-2 grid grid-cols-4 gap-1">
                            ...
                            <button
                                ...
                                :class="
                                    !depositPresets.includes(
                                        proposal.deposit_value as (typeof depositPresets)[number],
                                    )
                                        ? 'border-primary bg-primary/5 text-primary'
                ...
                >
                    ...
                    <p class="text-[11px] text-muted-foreground">
                        ...
                        <span class="font-semibold text-foreground">
                            <template
                                v-if="proposal.deposit_type === 'percentage'"
                            >
