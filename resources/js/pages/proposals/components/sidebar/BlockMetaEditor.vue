...
const updateMeta = (changes: Partial<BlockMeta>) => {
    ...
    store.updateBlockMetaRecursive(block.value.id, {
        ...meta.value,
        ...changes,
    });
};
...
const backgroundColorModel = computed({
    ...
    set: (value: string) => {
        updateMeta({
            background_color: value === 'transparent' ? null : value,
        });
    },
});
...
<template>
                ...
                <div class="grid grid-cols-5 gap-1">
                    <Tooltip v-for="opt in paddingOptions" :key="opt.value">
                        <TooltipTrigger as-child>
                            <button
                                type="button"
                                class="flex flex-col items-center gap-1 rounded-md border py-2 text-[11px] font-medium transition"
                                :class="
                                    meta.padding_top === opt.value
                    ...
                    <Tooltip v-for="opt in paddingOptions" :key="opt.value">
                        <TooltipTrigger as-child>
                            <button
                                ...
                                class="flex flex-col items-center gap-1 rounded-md border py-2 text-[11px] font-medium transition"
                                :class="
                                    meta.padding_bottom === opt.value
                                        ? 'border-primary bg-primary/5 text-primary'
                                        : 'border-border bg-background text-muted-foreground hover:border-muted-foreground hover:text-foreground'
                                "
                                @click="
                                    updateMeta({ padding_bottom: opt.value })
                                "
                            >
                                <span
                                    class="block w-4 rounded-sm transition-all"
            ...
            >
                ...
                >
                    <div>
                        ...
                        <p class="text-[11px] text-muted-foreground">
                            Thin separator line above block
                        </p>
                    </div>
                    <Switch
                        :model-value="meta.border_top ?? false"
                        @update:model-value="
                            (v: boolean) => updateMeta({ border_top: v })
                        "
                    />
                </label>
                <label
                ...
                >
                    <div>
                        ...
                        <p class="text-[11px] text-muted-foreground">
                            Thin separator line below block
                        </p>
                    </div>
                    <Switch
                        :model-value="meta.border_bottom ?? false"
                        @update:model-value="
                            (v: boolean) => updateMeta({ border_bottom: v })
                        "
                    />
                </label>
            </div>
        ...
        <div class="border-b border-border py-3">
            ...
            >
                ...
                <Switch
                    ...
                    @update:model-value="
                        (v: boolean) => updateMeta({ is_hidden: v })
                    "
                />
            </label>
            ...
            >
                This block is hidden. It will not appear in the client portal or
                PDF export.
            </p>
        </div>
    ...
    ══════════════════════════════════════════════ -->
        <div class="py-3">
            <button
                type="button"
                class="mb-3 flex w-full items-center justify-between text-[11px] font-semibold tracking-wider text-muted-foreground uppercase transition-colors hover:text-foreground"
            ...
            <template v-if="notesOpen">
                <Textarea
                    ...
                    @update:model-value="
                        (v) => updateMeta({ notes: v ? String(v) : null })
                    "
                />
