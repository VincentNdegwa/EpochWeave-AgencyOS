...
<template>
  ...
  <div class="-mx-4 -mb-4 flex h-[calc(100vh-64px)] flex-col bg-background lg:flex-row">
    <div class="relative flex-1 bg-muted/10">
      <ProposalCanvas :is-locked="true" :builder-mode="'proposal'" />
    </div>

    <aside class="w-full border-t border-border bg-background/95 p-6 shadow-2xl backdrop-blur lg:w-96 lg:border-t-0 lg:border-l">
      <div class="space-y-8">
        <section class="space-y-3">
          <div class="flex flex-col gap-2">
            <p class="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Proposal</p>
            <h1 class="text-2xl font-semibold leading-snug text-foreground">
            ...
            <p class="text-xs text-muted-foreground">
              Updated {{ formatDateTime(props.proposal.updated_at) }} • Views {{ props.proposal.view_count ?? 0 }}
            </p>
          </div>
          <div class="flex justify-end">
            <Link :href="proposalEdit(props.proposal.id).url" class="w-full">
        ...
        <section class="space-y-3">
          ...
          <div class="space-y-4">
            <div>
              <p class="text-xs uppercase text-muted-foreground">Total value</p>
              <p class="text-2xl font-semibold text-foreground">{{ formattedValue }}</p>
            </div>
            <div>
              <p class="text-xs uppercase text-muted-foreground">Deposit</p>
              <p class="text-sm text-foreground">{{ depositSummary }}</p>
            </div>
            <div>
              <p class="text-xs uppercase text-muted-foreground">Valid until</p>
              <p class="text-sm font-medium text-foreground">{{ validUntilDisplay }}</p>
        ...
        <section class="space-y-3">
          <p class="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Account</p>
          <Separator />
          <div class="flex items-center gap-2 text-sm font-medium">
            <Building2 class="h-4 w-4 text-muted-foreground" />
            <template v-if="accountHref">
              <Link :href="accountHref" class="text-primary underline-offset-4 hover:underline">
                {{ accountName }}
              </Link>
            </template>
            <template v-else>
        ...
        <section class="space-y-3">
          ...
          <div v-else class="rounded-lg border border-dashed border-border px-4 py-6 text-center text-xs text-muted-foreground">
